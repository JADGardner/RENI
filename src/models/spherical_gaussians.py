...
from utils.loss_functions import WeightedMSE
...
class SGEnvOptim:
    def __init__(
        ...
        self.thetaCenter = self.thetaCenter.reshape(1, self.SGNum, 1, 1, 1, 1).astype(
            np.float32
        )
        self.phiCenter = self.phiCenter.reshape(1, self.SGNum, 1, 1, 1, 1).astype(
            np.float32
        )
        ...
        lz = np.cos(El)
        self.ls = np.concatenate((lx, ly, lz), axis=2)[
        ...
        self.W = Variable(
            torch.from_numpy(
                np.sin(El.astype(np.float32)).reshape((1, 1, envHeight, envWidth))
            )
        )
        ...
        weight = Variable(
            torch.zeros((self.envNum, self.SGNum, self.ch), dtype=torch.float32)
        )
        ...
        lamb = torch.log(
            Variable(torch.ones(self.envNum, self.SGNum, 1) * np.pi / SGRow)
        )
        ...
        self.envmap = Variable(
            torch.FloatTensor(self.envNum, self.ch, self.envHeight, self.envWidth)
        )

        self.thetaCenter = self.thetaCenter.expand(
            [self.envNum, self.SGNum, 1, 1, 1, 1]
        )
    ...
    def renderSG(self, theta, phi, lamb, weight):
        ...
        mi = lamb.expand(
            [self.envNum, self.SGNum, 1, self.envHeight, self.envWidth]
        ) * (
            torch.sum(
                axis.expand(
                    [self.envNum, self.SGNum, 1, self.envHeight, self.envWidth, 3]
                )
                * self.ls.expand(
                    [self.envNum, self.SGNum, 1, self.envHeight, self.envWidth, 3]
                ),
        ...
        envmaps = weight.expand(
            [self.envNum, self.SGNum, self.ch, self.envHeight, self.envWidth]
        ) * torch.exp(mi).expand(
            [self.envNum, self.SGNum, self.ch, self.envHeight, self.envWidth]
        )
    ...
    def deparameterize(self):
        weight, theta, phi, lamb = torch.split(
            self.param.view(self.envNum, self.SGNum, 6), [3, 1, 1, 1], dim=2
        )
    ...
    def optimize(self, envmap, sineweight=None):
        assert (
            envmap.shape[0] == self.envNum
        ...
        for _ in range(0, self.niter):
            ...
            def closure():
                ...
                loss = WeightedMSE(
                    torch.log(recImage + 1), torch.log(self.envmap + 1), sineweight
                )
            ...
            if self.loss.cpu().data.item() < minLoss:
                if torch.isnan(torch.sum(self.param)):
                    ...
                    thetaBest = (
                        theta.data.cpu().numpy().reshape((self.envNum, self.SGNum, 1))
                    )
                    phiBest = (
                        phi.data.cpu()
                        .numpy()
                        .squeeze()
                        .reshape((self.envNum, self.SGNum, 1))
                    )
                    lambBest = (
                        lamb.data.cpu()
                        .numpy()
                    ...
                    weightBest = (
                        ...
                        .numpy()
                        .squeeze()
                        .reshape((self.envNum, self.SGNum, 3))
                    )
